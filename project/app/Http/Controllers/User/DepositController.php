...
class DepositController extends Controller {
    ...
    public function finalize($data)
    {
        try {
            ...
            return view('user.deposit.api_payment_submit',[
                'gateway'      => $gateway,
    ...
    public function depositPayment(Request $request)
    {
        ...
        $deposit_data['wallet'] = session('currency')->id;
        ...
        try {
            if($deposit['status'] == 1){
                ...
                if(session('api_depo')){
                    ...
                    if($request->type = 'manual') $gtype = 'manual';
                    else $gtype = 'auto';
                    return view('user.deposit.api_res',compact('gtype'));
                }
            ...
            if(session('api_depo')){
              return redirect(route('payment-init',['deposit_id' => session('api_depo')]))->with('error',__('Somthing went wrong please try again'));
            }
            return redirect(route('user.deposit.index'))->with('error',__('Somthing went wrong please try again'));
        }
        
        if(session('api_depo')){
            return redirect(route('payment-init',['deposit_id' => session('api_depo')]))->with('error',$deposit['message']);
        }
