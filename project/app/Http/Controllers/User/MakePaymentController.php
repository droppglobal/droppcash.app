...
class MakePaymentController extends Controller
{
    ...
    public function submitPayment(Request $request)
    {
        ...
        @mailSend('make_payment',["curr"=>$currency->code,'amount'=>amount($request->amount,$currency->type,3),"trnx"=>$trnx->trnx,"to_merchant"=>$receiver->email,'date_time'=> dateFormat( $receiverTrnx->created_at),"after_balance"=>amount($senderWallet->balance,$currency->type,3)],auth()->user());
         ...
         //mail to merchant
         @mailSend('receive_payment',["curr"=>$currency->code,'amount'=>amount($request->amount,$currency->type,3),"trnx"=>$trnx->trnx,"from_user"=>auth()->user()->email,'date_time'=> dateFormat( $receiverTrnx->created_at),"after_balance"=>amount($merchantWallet->balance,$currency->type,3),"charge"=>$finalCharge],$receiver);
